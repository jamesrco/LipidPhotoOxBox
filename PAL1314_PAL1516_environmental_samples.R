...
load("data/nice/Orbi_MS_data/LOBSTAHS_processed/PAL1314_LMG1401_particulate_enviro_samples_pos_withoddFA_LOBSet.RData")
PAL1314_LMG1401_particulate_pos_withoddFA = getLOBpeaklist(PAL1314_LMG1401_particulate_enviro_samples_pos_withoddFA) # generate peaklist
...
# create empty data frame
PAL1314_LMG1401_particulate.frag_lookup_classes = as.data.frame(matrix(NA,8,2))
rownames(PAL1314_LMG1401_particulate.frag_lookup_classes) = 
  c("PG","PE","PC","MGDG","SQDG","DGDG","DGCC","DGTS_DGTA")
colnames(PAL1314_LMG1401_particulate.frag_lookup_classes) =
  c("Frag_exp_type","mz_value")

# now, populate our data frame with necessary values
# per Popendorf et al., Lipids (2013) 48:185–195

PAL1314_LMG1401_particulate.frag_lookup_classes[,1] =
  c("CNL","CNL","PI","CNL","CNL","CNL","PI","PI")
PAL1314_LMG1401_particulate.frag_lookup_classes[,2] =
  c(189.040224,141.019094,184.073321,197.089937,261.051837,359.142212,104.106690,236.149249)

# pull in the xsAnnotate object & list containing the xcmsRaw objects

load("data/nice/Orbi_MS_data/xsAnnotate_objects/PAL1314_LMG1401_particulate_enviro_samples_pos_withoddFA_xsAnnotate.RData")
PAL1314_LMG1401_p_xsA_pos = PAL1314_LMG1401_particulate_enviro_samples_pos_withoddFA_xsAnnotate # rename so easier to work with

load("data/raw/Orbi_MS_data/xcmsRaw_objects/PAL1314_LMG1401_particulate_enviro_samples_pos_xcmsRaw.RData")
PAL1314_LMG1401_p_xsR = PAL1314_LMG1401_particulate_enviro_samples_pos_xcmsRaw

# preallocate two matrices for our results
# PAL1314_LMG1401.detected_pos_ion_fragments: to keep track of how many valid ms2 fragmentation spectra were detected for the feature in positive ion mode
# PAL1314_LMG1401.fragdata_results: number of ms2 spectra in which the PI or CNL criteria were validated

PAL1314_LMG1401.detected_pos_ion_fragments = as.data.frame(matrix(NA,nrow(PAL1314_LMG1401_particulate_pos_withoddFA),ncol=5))
PAL1314_LMG1401.fragdata_results = as.data.frame(matrix(NA,nrow(PAL1314_LMG1401_particulate_pos_withoddFA),ncol=5))
...
for (i in 1:(nrow(PAL1314_LMG1401_particulate_pos_withoddFA))) {
  ...
  this.ID = PAL1314_LMG1401_particulate_pos_withoddFA$compound_name[i]
  this.IDclass = PAL1314_LMG1401_particulate_pos_withoddFA$species[i]
  
  if (this.IDclass %in% rownames(PAL1314_LMG1401_particulate.frag_lookup_classes)) {
    # an escape if the lipid class isn't accounted for in our input parameter table
    
    # retrieve underlying positive-mode xcms group and peak data
    
    xcms.peakIDs_thisgroup_pos =
      PAL1314_LMG1401_p_xsA_pos@xcmsSet@groupidx[[PAL1314_LMG1401_particulate_pos_withoddFA$xcms_peakgroup[i]]]
    
    xcms.peakdata_thisgroup_pos =
      as.data.frame(PAL1314_LMG1401_p_xsA_pos@xcmsSet@peaks[xcms.peakIDs_thisgroup_pos,])
    ...
    for (j in 1:nrow(xcms.peakdata_thisgroup_pos)) {
      ...
      if (samp_ID %in% c(1:5)) {
        ...
        RT_min_pos.raw = PAL1314_LMG1401_p_xsA_pos@xcmsSet@rt$raw[[samp_ID]][
          PAL1314_LMG1401_p_xsA_pos@xcmsSet@rt$corrected[[samp_ID]]==
            xcms.peakdata_thisgroup_pos$rtmin[j]]
        
        RT_max_pos.raw = PAL1314_LMG1401_p_xsA_pos@xcmsSet@rt$raw[[samp_ID]][
          PAL1314_LMG1401_p_xsA_pos@xcmsSet@rt$corrected[[samp_ID]]==
            xcms.peakdata_thisgroup_pos$rtmax[j]]
        
        RT_ctr_pos.raw = PAL1314_LMG1401_p_xsA_pos@xcmsSet@rt$raw[[samp_ID]][
          PAL1314_LMG1401_p_xsA_pos@xcmsSet@rt$corrected[[samp_ID]]==
            xcms.peakdata_thisgroup_pos$rt[j]]
        ...
        valid.precursors_pos.ind =
          possible.precursors_pos.ind[PAL1314_LMG1401_p_xsR[[samp_ID]]@msnRt[possible.precursors_pos.ind]>
                                        (RT_min_pos.raw-20) &
                                        PAL1314_LMG1401_p_xsR[[samp_ID]]@msnRt[possible.precursors_pos.ind]<
                                        (RT_max_pos.raw+20)]
        
        # to get the actual QE scan numbers corresponding to this index, run the below:
        # PAL1314_LMG1401_p_xsR[[j]]@msnAcquisitionNum[valid.precursors_pos.ind]
        ...
        if (length(valid.precursors_pos.ind)>0) {
          ...
          if (is.na(PAL1314_LMG1401.detected_pos_ion_fragments[i,samp_ID])) {
            
            PAL1314_LMG1401.detected_pos_ion_fragments[i,samp_ID] = length(valid.precursors_pos.ind)
            
          } else {
            
            # there's already some data from a previous iteration; add to it
            
            PAL1314_LMG1401.detected_pos_ion_fragments[i,samp_ID] = 
              PAL1314_LMG1401.detected_pos_ion_fragments[i,samp_ID] + length(valid.precursors_pos.ind)
          ...
          if (is.na(PAL1314_LMG1401.detected_pos_ion_fragments[i,samp_ID])) {
            ...
            PAL1314_LMG1401.detected_pos_ion_fragments[i,samp_ID] = 0
        ...
        if (length(valid.precursors_pos.ind)>0) {
          ...
          if (PAL1314_LMG1401_particulate.frag_lookup_classes$Frag_exp_type[
            rownames(PAL1314_LMG1401_particulate.frag_lookup_classes)==this.IDclass] == "PI") {
            # this class is diagnosed via presence of a product ion
            ...
            if (is.na(PAL1314_LMG1401.fragdata_results[i,samp_ID])) {
              
              PAL1314_LMG1401.fragdata_results[i,samp_ID] = sum(unlist(PI.eval_result))
              
            } else {
              
              # there's already some data from a previous iteration; add to it
              
              PAL1314_LMG1401.fragdata_results[i,samp_ID] = 
                PAL1314_LMG1401.fragdata_results[i,samp_ID] + sum(unlist(PI.eval_result))
              
            }
            
          } else if (PAL1314_LMG1401_particulate.frag_lookup_classes$Frag_exp_type[
            rownames(PAL1314_LMG1401_particulate.frag_lookup_classes)==this.IDclass] == "CNL") {
            ...
            if (is.na(Marchetti_diatom.fragdata_results[i,samp_ID])) {
              ...
              PAL1314_LMG1401.fragdata_results[i,samp_ID] = sum(unlist(CNL.eval_result))
...
PAL1314_LMG1401_particulate_pos_withoddFA$ms2_conf =
  PAL1314_LMG1401.fragdata_result.summary

PAL1314_LMG1401.fragdata_presence.summary =
  apply(PAL1314_LMG1401.detected_pos_ion_fragments, 1, sumfrag)
